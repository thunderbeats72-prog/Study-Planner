...
// --- 2. CHOOSE YOUR AI ---
// Change the word below to "gemini", "groq", or "openrouter"
...
export async function callLLM(
  ...
  try {
    ...
    if (provider === "gemini") {
      ...
      // Both Groq and OpenRouter use standard OpenAI formatting
      let url = "";
      let key = "";
      let model = "";
...
function numberTheory(q: string): string | null {
  ...
  if (/\blcm\b|least common multiple/i.test(q) && nums.length >= 2) {
    ...
    return `**LCM of ${nums.join(", ")} = ${r}**\n\nUse LCM(a,b) = a  HCF(a,b). For more than two numbers, apply it pairwise.`;
  }
...
function derivative(q: string): string | null {
  ...
  return `**Power rule:** d/dx(ax ) = n \n\nTerm by term   **${parts.join(" + ").replace(/\+ -/g, "- ")}**`;
}

const CONCEPTS: Record<string, string> = {
  "spaced repetition": "**Spaced repetition** means reviewing material at increasing intervals (1 day   3 days   7 days   21 days) instead of cramming. Every time you *almost* forget something and then recall it, the memory trace gets stronger. Your planner already inserts automatic 48-hour and 1-week recall tasks after every lesson   those small 15-minute blocks are doing most of the heavy lifting for long-term retention.",
  "active recall": "**Active recall** = closing the book and forcing your brain to retrieve the answer. Re-reading feels productive but produces almost no durable learning. Practical routine: after each lesson, take a blank page and write everything you remember for 5 minutes, then compare with your notes and highlight only the gaps.",
  "pomodoro": "**Pomodoro** = 25 minutes of single-task focus + 5 minutes break, then a longer 15-minute break every 4 cycles. The point isn't the number 25   it's that a *bounded* session removes the dread of starting. Use the Focus Studio timer, pick the subject, and commit to just one cycle. Starting is the hard part.",
  "feynman": "**Feynman technique**: 1) Write the concept name at the top of a page. 2) Explain it as if teaching a 12-year-old. 3) Every time you stumble or reach for jargon, that's your gap   go back to the source. 4) Simplify and use an analogy. If you can't explain it simply, you don't understand it yet.",
  "interleaving": "**Interleaving** means mixing different topics/subjects in one session instead of blocking one topic for hours. It feels harder and slower   which is exactly why it works: your brain has to select the right strategy each time. Your planner interleaves subjects per day for this reason.",
  "procrastinat": "Procrastination is almost never a time-management problem   it's an **emotion-regulation** problem. The task feels ambiguous or threatening, so your brain avoids it. Fixes that actually work:\n\n1. **Shrink the unit**   commit to 10 minutes, not 3 hours.\n2. **Make step 1 stupidly concrete**   'open the book to page 84 and read one paragraph.'\n3. **Remove the phone from the room** (not just face-down).\n4. **Use implementation intentions**   'At 7pm, at my desk, I will do Lesson 3.'\n5. **Forgive the last miss.** Guilt increases avoidance; self-compassion reduces it.",
  "consistency": "Consistency comes from **lowering the activation energy**, not raising motivation. Three levers:\n\n1. **Anchor** the session to something you already do daily (after dinner   1 lesson).\n2. **Minimum viable day**   on bad days do a 15-minute recall task instead of skipping entirely. Never miss twice.\n3. **Visible streak**   the streak counter in your sidebar exists precisely for this.\n\nThe plan is designed with buffer days so one bad day never breaks the schedule.",
  "burnout": "Signs of burnout: dropping retention, irritability, dreading the desk, sleeping badly. Recovery protocol: take one **full** day off (guilt-free), then return at 50% load for two days. Reduce daily hours in Settings and hit *Re-plan with AI*   a smaller plan you actually follow beats a heroic plan you abandon.",
  "memory": "Memory tricks worth using: **chunking** (group items into 3 4s), **method of loci** (place items along a familiar walk), **acronyms** for ordered lists, **dual coding** (draw it as well as write it), and **sleep**   consolidation happens overnight, so a 7-hour night is a study technique.",
  "note": "Best note format for retention is the **Cornell layout**: main notes on the right, cue questions on the left, 3-line summary at the bottom. The cue column turns your notes into a self-test   which is the only part that actually builds memory.",
  "exam": "Exam-day protocol: 1) Two-pass strategy   sweep all easy marks first, then return to the hard ones. 2) Budget minutes per mark and check the clock at 25%/50%/75%. 3) Never leave a scoring question blank for a hard one. 4) Sleep beats a final all-nighter every single time.",
  "mock": "Mock tests are diagnostics, not scores. The 45 minutes *after* the mock matter more than the mock: classify each mistake as (a) concept gap, (b) silly error, (c) time pressure. Only (a) goes back on the study plan; (b) needs a checklist; (c) needs timed drills.",
  "swot": "**SWOT analysis** maps **S**trengths & **W**eaknesses (internal, controllable) against **O**pportunities & **T**hreats (external, uncontrollable). Practical use: don't just list   *cross* them. Strength   Opportunity = where to attack; Weakness   Threat = what to defend. In an exam, always finish with a strategic recommendation drawn from the grid, not just the four lists.",
  "marketing mix": "**The 4 Ps (Marketing Mix):** Product (features, quality, branding), Price (strategy, discounts, positioning), Place (channels, distribution, coverage), Promotion (advertising, PR, sales promotion, personal selling). Services add 3 more Ps: People, Process, Physical evidence. Exam tip: apply each P to the *specific* product in the question   generic definitions score low.",
  "stp": "**STP = Segmentation   Targeting   Positioning.** 1) *Segment* the market by demographics/psychographics/behaviour/geography. 2) *Target* the segment(s) worth serving (evaluate size, growth, competition, fit). 3) *Position* your offer in the target's mind via a differentiation and a positioning statement. It's the strategic step that comes *before* the 4 Ps.",
  "porter": "**Porter's Five Forces** assess industry attractiveness: 1) Threat of new entrants, 2) Bargaining power of suppliers, 3) Bargaining power of buyers, 4) Threat of substitutes, 5) Competitive rivalry. Strong forces = thin profits. Use it to explain *why* an industry is (un)profitable and where a firm should build defences.",
  "demand": "**Law of demand:** as price rises, quantity demanded falls (ceteris paribus), because of the substitution and income effects. Draw the downward-sloping curve; distinguish a *movement along* it (price change) from a *shift* of it (income, tastes, substitutes, expectations). Elasticity tells you how sensitive quantity is to price.",
  "elasticity": "**Price elasticity of demand (PED)** = %  quantity   price. |PED|>1 elastic (luxuries), <1 inelastic (necessities), =1 unit elastic. Key exam link: if demand is inelastic, a price *rise* increases total revenue; if elastic, a price *cut* does. Determinants: substitutes, necessity, proportion of income, time horizon.",
  "photosynthesis": "**Photosynthesis** converts light energy into chemical energy: 6CO  + 6H O + light   + 6O . Two stages: 1) *Light reactions* in the thylakoid   split water, make ATP & NADPH, release O . 2) *Calvin cycle* in the stroma   fix CO  using ATP/NADPH to build glucose. Exam trap: light reactions need light directly; the Calvin cycle uses the products of the light reactions, not light itself.",
  "newton": "**Newton's three laws:** 1) *Inertia*   a body stays at rest/uniform motion unless acted on by a net force. 2) *F = ma*   net force = mass   acceleration (the workhorse for numericals). 3) *Action reaction*   forces come in equal, opposite pairs on different bodies. Always draw a free-body diagram before applying law 2.",
  "supply and demand": "**Market equilibrium** is where the supply and demand curves cross   the price where quantity supplied = quantity demanded. Above it, surplus pushes price down; below it, shortage pushes price up. Shifts in either curve create a new equilibrium; practise predicting the direction of price and quantity for each shift.",
};
...
export async function aiSuggestSubjects(
  ...
  const raw = await callLLM(
    ...
    [
      {
        role: "user",
        content: `Course/exam: "${courseName}". Education level: ${level}. List the 4-7 real subjects/papers a student actually studies for this, as a JSON array: [{"name":"...","units":8,"difficulty":"Easy|Medium|Hard","color":"#6366f1"}] "units" = number of major chapters/lessons in that subject (3-20). JSON array only.`,
      },
    ],
    800
  );
