...
/* One controller for Focus + Study Clock. Focus-owned sessions keep the two
   timers synchronized; manual clock sessions remain independent. */
...
export function planSession(snapshot: SessionSnapshot, command: SessionCommand): SessionEffect[] {
  ...
  switch (command.type) {
    ...
    case "setMode": {
      ...
      if (snapshot.focusOwnsClock && isBreakMode(command.mode) && snapshot.clockSessionActive && !snapshot.clockOnBreak) {
        fx.push({ kind: "clock.break" });
      }
