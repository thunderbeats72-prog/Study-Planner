  ...
  return <div className="fade-in focus-view">
    ...
    <div className="focus-grid-2 rv">
      <section className="glass-panel tilt-card liquid-card flex-col section-card timer-panel" aria-labelledby="focus-timer-title">
        ...
        <div className="timer-stage"><div className={`timer-ring-wrap${timer.running ? " is-running" : ""}${timer.isBreak ? " is-break" : ""}`} role="timer" aria-label={`${mmss(timer.seconds)} ${timerStateLabel.toLowerCase()}`}><svg viewBox="0 0 240 240" aria-hidden="true">{/* Gradient ring: a soft accent → violet sweep while focused, solid mint on a break. The running glow lives purely in CSS. */}<defs><linearGradient id="focusRingGrad" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="var(--accent)" /><stop offset="100%" stopColor="var(--accent-strong, #8b5cf6)" /></linearGradient></defs><circle className="timer-ring-track" cx="120" cy="120" r="104" stroke="var(--row-bg)" strokeWidth="10" fill="transparent" /><circle className="timer-ring-prog" cx="120" cy="120" r="104" stroke={timer.isBreak ? "var(--success-accent)" : "url(#focusRingGrad)"} strokeWidth="10" fill="transparent" strokeDasharray={circ} strokeDashoffset={circ * (1 - pct)} strokeLinecap="round" style={{ transition: "stroke-dashoffset .4s linear" }} /></svg><div className="timer-center"><div id="t-digits" className="mono">{mmss(timer.seconds)}</div><div id="t-label">{timerStateLabel}</div></div></div></div>
